...
class BILSTM_Model(object):
    def __init__(self, vocab_size, out_size, crf=True):
        ...
        self.n_layers = LSTMConfig.n_layers

        self.crf = crf
        # 根据是否添加crf初始化不同的模型 选择不一样的损失计算函数
        if not crf:
            self.model = BiLSTM(vocab_size, self.emb_size,
                                self.hidden_size, self.n_layers, out_size).to(self.device)
    ...
    def get_model_pred(self, word_lists, word2id, tag2id):
        """返回最佳模型在测试集上的预测结果"""
...
class BiLSTM_CRF(nn.Module):
    def __init__(self, vocab_size, emb_size, hidden_size, n_layers, out_size):
        ...
        self.bilstm = BiLSTM(vocab_size, emb_size, hidden_size, n_layers, out_size)
